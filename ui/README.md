...
### Advanced Scenarios
The above steps will display your Azure Resources, but that's just the beginning. Let's say you implemented a `browse` function on your `WebAppTreeItem` that opened the Web App in the browser. In order to make that command work from the VS Code command palette, use the `showTreePicker` method:
```typescript
registerCommand('appService.Browse', async (treeItem?: WebAppTreeItem) => {
    if (!treeItem) {
        treeItem = <WebAppTreeItem>await treeDataProvider.showTreePicker(WebAppTreeItem.contextValue);
    }
